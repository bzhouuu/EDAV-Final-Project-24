...
ggplot(artist_age, aes(x = Classification, y = Age)) + 
...
# manually drop rows as before
age_gender <- age_total %>% group_by(Age) %>% filter(Age >= 15 & Age <= 70)

# plot boxplots for gender
ggplot(age_gender, aes(x = Gender, y = Age)) + 
  geom_boxplot() +
  ggtitle("MOMA Preferred Age to Create Art (15 - 70 yrs Old)", subtitle = "Boxplots of ages when pieces are created, by Gender") +
  labs(x = "Gender", y = "Age") +
  theme(plot.title = element_text(face = "bold")) +
  theme(plot.subtitle = element_text(face = "bold", color = "grey35")) +
  theme(plot.caption = element_text(color = "grey68"))
