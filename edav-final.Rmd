...
moma_artists_nat_5per <- moma_artists_nat_5per %>% group_by(Nationality) %>% filter(Nationality %in% c("American","German","French","British","Italian","Japanese"))
...
# filter out NA for now
moma_artists_nat_10per <- moma_artists_nationality %>% group_by (Nationality) %>% filter(Nationality %in% c("American", "German","French","British", "Italian","Japanese","Swiss","Dutch","Russian","Austrian","Canadian"))
