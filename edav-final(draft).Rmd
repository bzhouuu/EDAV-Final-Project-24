...
met <- read_csv("./data/MetObjects.csv")
tate_artworks <- read_csv("./data/tate_artwork.csv")
tate_artists <- read_csv("./data/tate_artist.csv")
...
moma_artists_birth_year_gender <- moma_artists[4:5] %>% group_by(Gender, BeginDate) %>% dplyr::summarize(Frequency=n())
...
tate_test <- tate %>% group_by(acquisitionYear) %>% dplyr::summarise(Freq = n()) %>% arrange(Freq)
